...
const columns: ColumnDef<TeamStanding>[] = [
  {
    ...
    cell: ({ row }) => (
      <div className="flex items-center gap-2">
        ...
        <span className="font-[family-name:var(--font-heading)] font-bold text-[#3D2B1F]">{row.original.team}</span>
        <span className="font-[family-name:var(--font-display)] text-[10px] text-[#8B7355]">{row.original.abbreviation}</span>
      </div>
    ),
    enableSorting: false,
  },
  ...
  { accessorKey: "pct", header: "PCT", cell: ({ getValue }) => <span className="font-[family-name:var(--font-mono)] font-bold text-[#0D2240]">{getValue<string>()}</span> },
  { accessorKey: "rs", header: "RS", cell: ({ getValue }) => <span className="font-[family-name:var(--font-mono)]">{getValue<number>()}</span> },
  { accessorKey: "ra", header: "RA", cell: ({ getValue }) => <span className="font-[family-name:var(--font-mono)]">{getValue<number>()}</span> },
  {
    accessorKey: "diff",
    header: "DIFF",
    cell: ({ getValue }) => {
      ...
      return (
        <span className={cn("font-[family-name:var(--font-mono)] font-bold", v.startsWith("+") ? "text-[#006400]" : v.startsWith("-") ? "text-[#C41E3A]" : "text-[#3D2B1F]")}>
          {v}
        </span>
      )
    },
  },
  { accessorKey: "home", header: "HOME", cell: ({ getValue }) => <span className="font-[family-name:var(--font-mono)]">{getValue<string>()}</span> },
  { accessorKey: "away", header: "AWAY", cell: ({ getValue }) => <span className="font-[family-name:var(--font-mono)]">{getValue<string>()}</span> },
  { accessorKey: "l10", header: "L10", cell: ({ getValue }) => <span className="font-[family-name:var(--font-mono)]">{getValue<string>()}</span> },
  {
    accessorKey: "strk",
    header: "STRK",
    cell: ({ getValue }) => {
      ...
      return (
        <span className={cn(
          "font-[family-name:var(--font-mono)] text-xs font-bold px-1.5 py-0.5 rounded-sm",
          v.startsWith("W") ? "bg-[#0D2240] text-[#F5C842]" : "bg-[#C41E3A] text-[#FDF6E3]"
        )}>
          {v}
        </span>
      )
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {divisions.map((d) => (
          <button
            ...
            className={cn(
              "shrink-0 px-4 py-1.5 text-xs font-[family-name:var(--font-display)] uppercase tracking-wider rounded-sm border-2 transition-colors",
              division === d
                ? "bg-[#0D2240] text-[#F5C842] border-[#0D2240]"
                : "bg-[#FDF6E3] text-[#8B7355] border-[#8B7355] hover:bg-[#F5E6C8] hover:text-[#3D2B1F]"
            )}
          >
            {d}
          </button>
